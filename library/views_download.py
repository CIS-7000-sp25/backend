...
from library.models import Asset, Commit, Sublayer
...
def download_asset_by_version(request, asset_name, version):
    ...
    for sublayer in commit.sublayers.all():
        ...
        truncated_key = s3_key[s3_key.find(asset_name):] # truncates key to beginning of first occurence of asset_name
    ...
    response['Content-Disposition'] = f'attachment; filename="{asset_name}_{version}.zip"'
    return response
